...
opp_stats <- read_rds("../Data/stats_vs_opp_today.rds")

# Normalise rolling averages to percentile
opp_stats <-
opp_stats |> 
  group_by(oppositionTeam) |>
  slice_tail(n = 1) |> 
  ungroup() |>
  mutate_at(vars(contains("rolling")), ~rank(.)/max(rank(.))) |>
...
all_player_points <-
  ...
  mutate(fixture_difficulty = rolling_10_game_points)

all_player_assists <-
  all_player_assists |>
  left_join(opp_stats, by = c("opposition_team" = "oppositionTeam")) |> 
  mutate(fixture_difficulty = rolling_10_game_assists)

all_player_rebounds <-
  all_player_rebounds |>
  left_join(opp_stats, by = c("opposition_team" = "oppositionTeam")) |> 
  mutate(fixture_difficulty = rolling_10_game_rebounds)

all_player_threes <-
  all_player_threes |>
  left_join(opp_stats, by = c("opposition_team" = "oppositionTeam")) |> 
  mutate(fixture_difficulty = rolling_10_game_threes)

all_player_steals <-
  all_player_steals |>
  left_join(opp_stats, by = c("opposition_team" = "oppositionTeam")) |> 
  mutate(fixture_difficulty = rolling_10_game_steals)

all_player_blocks <-
  all_player_blocks |>
  left_join(opp_stats, by = c("opposition_team" = "oppositionTeam")) |> 
  mutate(fixture_difficulty = rolling_10_game_blocks)

all_player_pras <-
  all_player_pras |>
  left_join(opp_stats, by = c("opposition_team" = "oppositionTeam")) |> 
  mutate(fixture_difficulty = rolling_10_game_PRAs)
...
player_points_overs_to_bet <-
  ...
  filter(over_price >= 1.3) |> filter(fixture_difficulty >= 0.45)
...
player_points_unders_to_bet <-
  ...
  filter(under_price >= 1.3) |> filter(fixture_difficulty <= 0.55)
