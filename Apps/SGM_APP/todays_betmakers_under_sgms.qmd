---
title: "Today's Betmaker Under SGMs"
execute:
  echo: false
  message: false
  warning: false
...
points_rebounds_to_bet <-
  ...
  separate(Selections, into = c("points_line", "rebounds_line"), sep = ", ", remove = FALSE) |>
  mutate(points_line = str_extract(points_line, "\\d+\\.\\d+")) |>
  mutate(rebounds_line = str_extract(rebounds_line, "\\d+\\.\\d+")) |>
  mutate(points_line = as.numeric(points_line)) |>
  mutate(rebounds_line = as.numeric(rebounds_line)) |>
  filter(rebounds_line > 4.5) |> 
